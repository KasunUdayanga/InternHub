            ...
            {userApplication?.map((intern, index) => (
              <tr key={index} className="text-center">
                <td className="border-b px-4 py-2 flex items-center gap-2">
                  <img src={intern.companyId?.image || assets.default_logo} alt="Company logo" className="w-8 h-8" />
                  {intern.companyId?.name || "Unknown Company"}
                </td>
                <td className="border-b px-4 py-2">{intern.internId?.title || "N/A"}</td>
                <td className="border-b px-4 py-2">{intern.internId?.location || "N/A"}</td>
                <td className="border-b px-4 py-2">
                  {intern.date ? moment(intern.date).format('ll') : "N/A"}
                </td>
                <td className="border-b px-4 py-2">
